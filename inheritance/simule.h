...
public:
    ...
    void inheritance ( )
    {
    	...
    	for( int i=0 ; i < sz; i++)
    			{
    				if (locking_process->get_priority() > sem_q.at(i).get_priority() ) 
	    				{
	    					locking_process->set_priorrity( sem_q.at(i).get_priority() );
	    					cout <<"Process"<< locking_process->getpid()<< " inherited priority "<< locking_process->get_priority()<<endl;
	    				}
